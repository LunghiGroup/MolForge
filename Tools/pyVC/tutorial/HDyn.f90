        ...
        double precision                :: step=1.0d0,time=0.0d0,temp_rho=20.0d0,val(3),num
        ...
        integer                            :: N,M,NBl,MBl,numroc,info,ii,jj,raman
...
!         sp%nderiv=234
         ...
         open(12,file='grad_matrix_R.txt')
         open(13,file='grad_matrix_C.txt')
         read(12,*)  
         read(13,*) 
...
!          enddo
         ...
         do i=1,sp%nderiv
          do s=1,sp%Hdim
           do t=s+1,sp%Hdim
            Vx(i,s,t)=(Vx(i,s,t)+conjg(Vx(i,t,s)))/2.0d0
            Vx(i,t,s)=conjg(Vx(i,s,t))
           enddo
          enddo
         enddo
   ...
   ! Make Vx
         ...
         do i=1,sp%nderiv
          ...
          trace=(0.0d0,0.0d0)
          do ii=1,sp%Hdim
           trace=trace+sp%Vx(i)%mat(ii,ii)
          enddo
          do ii=1,sp%Hdim
!           sp%Vx(i)%mat(ii,ii)=sp%Vx(i)%mat(ii,ii)-trace/sp%Hdim
            sp%Vx(i)%mat(ii,ii)=0
          enddo

!          sp%Vx(i)%mat(1,2)=(0.0d0,0.0d0)
!          sp%Vx(i)%mat(2,1)=(0.0d0,0.0d0)
...
!         call sp%make_R41_lindbladian(min_ener=5.0d0,max_ener=500.0d0)
         if(raman.eq.0)then
          call sp%make_R21_lindbladian(min_ener=5.0d0,max_ener=3000.0d0)
         else if(raman.eq.1)then
          call sp%make_R41_lindbladian(min_ener=5.0d0,max_ener=3000.0d0)
         endif
          

         call sp%diag_lindbladian(step)
