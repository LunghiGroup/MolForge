#! /bin/bash
 ...
 then
  ...
  ${CF_DIR}/CF.x -JMult ${Nj} -lmax ${lmax} -CISIZE ${soc_size} -spin_only -read_dH -Bz ${field}

 else

  ${CF_DIR}/CF.x -JMult ${Nj} -lmax ${lmax} -CISIZE ${soc_size} -rot ${alpha} ${beta} ${gamma} -spin_only -read_dH -Bz ${field}
