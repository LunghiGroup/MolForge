...
!               this%dos2pm%get_val(abs(DEner2)).lt.1.0d-6 ) cycle
      ...
      ! Make Rij
         ...
         R0%mat=pi*pi*R0%mat*step_min*mult_fact/hplank/2.0d0
...
!           if (this%dos2pm%get_val(phondy%list(ph)%freq(bn)).lt.1.0d-6) cycle
      ...
      ! Build SPH per phonon
           ...
           write(*,*) ph,bn,phondy%list(ph)%freq(bn),dble(this%SPH%DSI(1)%D(1,:))**2,dble(this%SPH%DSI(1)%D(2,:)),dble(this%SPH%DSI(1)%D(3,:))**2
...
!         call mat_inv(BB%mat,this%Hdim) 
      ...
      ! check spectrum overlap
           ...
           if ( (ph-1)*size(phondy%list(ph)%freq)+bn .ge. &
